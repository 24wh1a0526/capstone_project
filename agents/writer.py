...
If change-tracking data is provided, summarize what's new and what's updated
since the previous briefing on this topic, each with a citation. If this is
the first briefing for this topic (no prior data), write exactly:
"This is the first briefing for this topic — no prior baseline to compare against."
...
"""
    ...
    def _format_analysis(self, analysis: Dict[str, Any]) -> str:
        ...
        changes = analysis.get("changes_since_last_briefing")
        if changes and (changes.get("new") or changes.get("updated")):
            parts.append("\nCHANGES SINCE LAST BRIEFING:")
            for item in changes.get("new", []):
                citation = item.get("citation", {})
                parts.append(
                    f"  - NEW: {item.get('summary', '')} "
                    f"[Source: {citation.get('title', '')}]({citation.get('url', '')})"
                )
            for item in changes.get("updated", []):
                citation = item.get("citation", {})
                parts.append(
                    f"  - UPDATED: {item.get('summary', '')} "
                    f"(was: {item.get('was', '')} -> now: {item.get('now', '')}) "
                    f"[Source: {citation.get('title', '')}]({citation.get('url', '')})"
                )
            if changes.get("unchanged_count"):
                parts.append(f"  - {changes['unchanged_count']} prior findings reconfirmed, unchanged.")
        elif changes is not None:
            parts.append("\nCHANGES SINCE LAST BRIEFING: None recorded (or first briefing for this topic).")
        ...
        while i < len(lines):
            ...
            if current_section in sections_to_check:
                ...
                if j < len(lines) and lines[j].strip().startswith("##"):
                    if current_section == "## What Changed Since Last Briefing":
                        notice = "This is the first briefing for this topic — no prior baseline to compare against."
                    else:
                        notice = "Unverified information — omitted from final report."
