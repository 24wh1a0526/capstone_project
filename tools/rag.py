...
# Caps how much of each source's content actually gets embedded. Embedding
# every character of every source is the single biggest hidden token spend
# in this pipeline — lowering this cuts embedding cost per run substantially
# with only a small loss in RAG retrieval depth.
RAG_MAX_CHARS_PER_DOC = int(os.getenv("RAG_MAX_CHARS_PER_DOC", 1200))
...
class RAGTool:
    ...
    def _prepare_chunks(self, source_documents: List) -> tuple[List[str], List[dict]]:
        ...
        for doc in source_documents:
            ...
            # Cap how much of this source gets embedded — the single biggest
            # lever for reducing embedding cost per run.
            content = content[:RAG_MAX_CHARS_PER_DOC]
